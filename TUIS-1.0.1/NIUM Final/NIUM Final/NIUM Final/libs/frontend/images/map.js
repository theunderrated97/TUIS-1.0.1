...
function legend() {
    ...
    overlays.getLayers().getArray().slice().forEach(layer => {
        ...
        element.appendChild(head);
        var img = new Image();
        img.src = "http://3.110.240.123:8080/geoserver/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&WIDTH=20&HEIGHT=20&LAYER=" + layer.get('title');
...
$(document).ready(function() {
    $.ajax({
        type: "GET",
        url: "http://3.110.240.123:8080/geoserver/wfs?request=getCapabilities",
        dataType: "xml",
...
$(function() {
    $("#layer").change(function() {
        ...
        $(document).ready(function() {
            $.ajax({
                type: "GET",
                url: "http://3.110.240.123:8080/geoserver/wfs?service=WFS&request=DescribeFeatureType&version=1.1.0&typeName=" + value_layer,
                dataType: "xml",
                success: function(xml) {
...
function query() {
    ...
    //alert(value_txt);
    var url = "http://3.110.240.123:8080/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + value_layer + "&CQL_FILTER=" + value_attribute + "%20" + value_operator + "%20" + value_txt + "&outputFormat=application/json"
    //console.log(url);
...
function wms_layers() {
    ...
    $(document).ready(function() {
        $.ajax({
            type: "GET",
            url: "http://3.110.240.123:8080/geoserver/wms?request=getCapabilities",
            dataType: "xml",
            success: function(xml) {
...
function add_layer() {
    ...
    var layer_wms = new ol.layer.Image({
        ...
        source: new ol.source.ImageWMS({
            url: 'http://3.110.240.123:8080/geoserver/wms',
            params: {
                'LAYERS': layer_name
            },
            ratio: 1,
    ...
    var url = 'http://3.110.240.123:8080/geoserver/wms?request=getCapabilities';
...
function info() {
    if (document.getElementById("info_btn").innerHTML == "Info") {
        ...
        document.getElementById("info_btn").innerHTML = "X Info";
        ...
        document.getElementById("info_btn").setAttribute("class", "btn-sm");
        if (popup) {
            popup.hide();
        }
    }
...
function show_hide_querypanel() {
    ...
    if (document.getElementById("query_tab").style.visibility == "hidden") {
        ...
        document.getElementById("query_panel_btn").innerHTML = "Close Query";
        // document.getElementById("query_panel_btn").setAttribute("class", "btn ");
    ...
    if (value !== 'None') {
		...
		draw1 = new ol.interaction.Draw({
            ...
            draw1.on('drawend', function(evt) {
                ...
                var url = "http://3.110.240.123:8080/geoserver/wfs?request=GetFeature&version=1.0.0&typeName=" + value_layer + "&outputFormat=json&cql_filter=INTERSECTS(the_geom," + wkt + ")";
                //alert(url);
